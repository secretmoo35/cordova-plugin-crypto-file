...
public class IonicWebViewEngine extends SystemWebViewEngine {
  ...
  private static final String LAST_BINARY_VERSION_CODE =
    "lastBinaryVersionCode";
  private static final String LAST_BINARY_VERSION_NAME =
    "lastBinaryVersionName";
  ...
  public void init(
    ...
    final CordovaWebViewEngine.Client client,
    ...
    SharedPreferences prefs = cordova
      .getActivity()
      .getApplicationContext()
      .getSharedPreferences(
        IonicWebView.WEBVIEW_PREFS_NAME,
        Activity.MODE_PRIVATE
      );
    String path = prefs.getString(IonicWebView.CDV_SERVER_PATH, null);
    if (
      !isDeployDisabled() && !isNewBinary() && path != null && !path.isEmpty()
    ) {
      setServerBasePath(path);
    }

    boolean setAsServiceWorkerClient = preferences.getBoolean(
      ...
      controller.setServiceWorkerClient(
        new ServiceWorkerClient() {
          ...
          public WebResourceResponse shouldInterceptRequest(
            ...
            return localServer.shouldInterceptRequest(
              request.getUrl(),
              request
            );
          }
  ...
  private boolean isNewBinary() {
    ...
    try {
      PackageInfo pInfo =
        this.cordova.getActivity()
          .getPackageManager()
          .getPackageInfo(this.cordova.getActivity().getPackageName(), 0);
    ...
    if (
      !versionCode.equals(lastVersionCode) ||
  ...
  private class ServerClient extends SystemWebViewClient {
    ...
    public WebResourceResponse shouldInterceptRequest(
      ...
      WebResourceResponse r = localServer.shouldInterceptRequest(
        request.getUrl(),
        request
      );
    ...
    public void onPageStarted(WebView view, String url, Bitmap favicon) {
      ...
      if (
        !launchUrl.contains(WebViewLocalServer.httpsScheme) &&
        !launchUrl.contains(WebViewLocalServer.httpScheme) &&
        url.equals(launchUrl)
      ) {
        view.stopLoading();
        // When using a custom scheme the app won't load if server start url doesn't end in /
        String startUrl = CDV_LOCAL_SERVER;
        if (
          !scheme.equalsIgnoreCase(WebViewLocalServer.httpsScheme) &&
