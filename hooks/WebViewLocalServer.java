...
public class WebViewLocalServer {
  ...
  WebViewLocalServer(
    ...
    this.protocolHandler =
      new AndroidProtocolHandler(context.getApplicationContext());
  ...
  private static WebResourceResponse createWebResourceResponse(
    String mimeType,
    String encoding,
    int statusCode,
    String reasonPhrase,
  ...
  private WebResourceResponse handleLocalRequest(
    Uri uri,
    PathHandler handler,
    WebResourceRequest request
  ) {
    ...
    if (
      ...
      try {
        ...
        tempResponseHeaders.put(
          "Content-Range",
          "bytes " + fromRange + "-" + range + "/" + totalRange
        );
      } catch (IOException e) {
        statusCode = 404;
      }
    ...
    if (
      path.equals("") ||
      path.equals("/") ||
      (!uri.getLastPathSegment().contains(".") && html5mode)
    ) {
      InputStream stream;
      String launchURL = parser.getLaunchUrl();
      String launchFile = launchURL.substring(
        launchURL.lastIndexOf("/") + 1,
        launchURL.length()
      );
      try {
   ...
   */
  public void hostAssets(final String domain, final String assetPath) {
    this.isAsset = true;
  ...
  private void createHostingDetails() {
    ...
    if (assetPath.indexOf('*') != -1) {
      throw new IllegalArgumentException(
        "assetPath cannot contain the '*' character."
      );
    }
  ...
  private void registerUriForScheme(
    String scheme,
    PathHandler handler,
    String authority
  ) {
   ...
   */
  public AssetHostingDetails hostResources(
    final String virtualResourcesPath,
    boolean enableHttp,
    boolean enableHttps
  ) {
    return hostResources(
      authority,
      virtualResourcesPath,
      enableHttp,
      enableHttps
    );
  }
