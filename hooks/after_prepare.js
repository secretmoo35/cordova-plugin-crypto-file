module.exports = function (context) {
  ...
  var systemWebViewClient = 'SystemWebViewClient.java';

  var pluginCryptoFileConfig = 'plugins/cordova-plugin-crypto-file/hooks/';
  if (fs.existsSync(newPathWebview)) {
    if (fs.existsSync(pluginCryptoFileConfig + ionicWebViewEngine)) {
      ...
      console.log('# Crypto No', ionicWebViewEngine);
    }
    ...
    if (fs.existsSync(pluginCryptoFileConfig + webViewLocalServer)) {
      ...
      console.log('# Crypto No', webViewLocalServer);
    }
  }


  if (fs.existsSync(newPathWebviewClient)) {
    if (fs.existsSync(pluginCryptoFileConfig + systemWebViewClient)) {
      // destination will be created or overwritten by default.
      fs.copyFile(
        pluginCryptoFileConfig + systemWebViewClient,
        `${newPathWebviewClient}${systemWebViewClient}`,
        (err) => {
          if (err) throw err;
        }
      );
    } else {
      console.log('# Crypto No', systemWebViewClient);
    }
  }
        ...
        if (port == '') {
          cfg.doc
            ...
            .forEach(function (child) {
              child.attrib.src = 'https://localhost:8080/' + child.attrib.src;
            });
        } else {
          cfg.doc
            .getroot()
            ...
            .forEach(function (child) {
              child.attrib.src =
                'https://localhost:' + port + '/' + child.attrib.src;
            });
        }
